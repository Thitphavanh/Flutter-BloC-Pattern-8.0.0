...
import 'package:flutter_bloc_plattern/models/todos_filter_model.dart';
    ...
    return BlocConsumer<TodosFilterBloc, TodosFilterState>(
      listener: (context, state) {
        if (state is TodosFilterLoaded) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'There are ${state.filteredTodos.length} To Dos in your ${state.todosFilter.toString().split('.').last} last.'),
            ),
          );
        }
      },
      builder: (context, state) {
        ...
        if (state is TodosFilterLoaded) {
          ...
          return const Text('ມີຂໍ້ຜິດພາດເກີດຂຶ້ນ !!!');
        }
      },
    );
  }
